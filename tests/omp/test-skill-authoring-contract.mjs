...
test('skill authoring covers invocation class, context pointer, completion, pruning, pressure', async () => {
  ...
  assert.ok(
    hasAffirmative(text, /context pointer|Load this reference when|precise (context )?pointer|progressive disclosure/i),
    'precise context pointer for references',
  );
  assert.ok(
  ...
  assert.ok(
    hasAffirmative(text, /prun(e|ing)|no-op|duplicat|sediment|sprawl|delete .*dead|remove .*obsolete/i),
...
test('authoring self-applies hierarchy without exhaustive per-skill pressure corpus', async () => {
  ...
  assert.ok(
    hasAffirmative(text, /hierarch|one leading concept|SKILL\.md first|progressive disclosure/i),
  ...
  assert.ok(
    hasAffirmative(text, /changed (routing|route|handoff|hierarchy|completion)|only .+ changed/i) ||
      /pressure-test only changed/i.test(text) ||
      /not .+ every skill|without exhaustive/i.test(text),
    'pressure tests target changed routes, not full corpus',
  );
});
...
test('retained writing-skills references declare load conditions or are dropped', async () => {
  ...
  assert.ok(text.length < 25_000, 'SKILL.md stays bounded');
  ...
  for (const ref of refs) {
    if (['render-graphs.js', 'graphviz-conventions.dot'].includes(ref)) {
      ...
      if (text.includes(ref)) {
        assert.match(
          text,
          new RegExp(`${ref.replace('.', '\\.')}[\\s\\S]{0,120}(when|only|if)|Load .+ when[\\s\\S]{0,80}${ref.replace('.', '\\.')}`, 'i'),
    ...
    if (text.includes(ref) || text.includes(ref.replace(/\.md$/, ''))) {
      const loadOk =
        new RegExp(`Load this reference when|load .+${ref.replace('.', '\\.')}|${ref.replace('.', '\\.')}.{0,80}when`, 'i').test(text) ||
